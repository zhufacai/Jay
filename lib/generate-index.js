...
// 配置参数 - 通过环境变量设置
const GITHUB_REPO = process.env.GITHUB_REPO || 'zhufacai/ceshi'; // 格式: owner/repo
...
async function fetchMusicFiles() {
  try {
    ...
    console.log(`是否有 Token: ${GITHUB_TOKEN ? '是' : '否'}`);
    ...
    // 总是尝试从 GitHub 获取数据，无论是否有 token
    try {
      console.log('尝试从 GitHub 获取音乐文件...');
      musicData = await getMusicFilesFromGitHub();
    } catch (error) {
      console.error('从 GitHub 获取数据失败:', error.message);
      
      // 如果获取失败，创建示例数据用于测试
      musicData = createSampleData();
    }
    ...
    console.log(`音乐数据已保存到 public/music-data.json`);
    console.log(`专辑数量: ${musicData.totalAlbums}`);
    console.log(`歌曲数量: ${musicData.totalSongs}`);
    
    // 创建或更新 index.html
    const indexPath = path.join(publicDir, 'index.html');
    createModernIndexHtml(indexPath, musicData);
    console.log('已创建/更新 index.html');
    ...
    const errorData = {
      ...
      path: MUSIC_PATH || '根目录',
      error: error.message,
      message: '请配置 GITHUB_TOKEN 环境变量以访问 GitHub 仓库'
    };
    ...
    const indexPath = path.join(publicDir, 'index.html');
    createModernIndexHtml(indexPath, errorData);
    console.log('已创建默认 index.html（无音乐文件）');
  }
}

// 直接从 GitHub API 获取音乐文件
async function getMusicFilesFromGitHub() {
  try {
    ...
    if (GITHUB_TOKEN) {
      ...
      console.warn('警告: 未提供 GITHUB_TOKEN，API 调用将受限制');
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 403 && !GITHUB_TOKEN) {
        throw new Error('GitHub API 限制，请设置 GITHUB_TOKEN 环境变量');
      } else if (response.status === 404) {
        throw new Error(`仓库或路径不存在: ${GITHUB_REPO}/${MUSIC_PATH}`);
      } else {
    ...
    // 处理每个文件夹（专辑）- 最多处理12个文件夹
    const maxFolders = Math.min(folders.length, 12);
    console.log(`处理前 ${maxFolders} 个文件夹...`);
    
    for (let i = 0; i < maxFolders; i++) {
      const folder = folders[i];
      try {
        console.log(`处理文件夹 ${i + 1}/${maxFolders}: ${folder.name}`);
    ...
    console.error('从 GitHub 获取音乐文件时出错:', error);
    throw error;
  }
}
...
function createSampleData() {
  ...
  for (let i = 1; i <= albumCount; i++) {
    const tracks = [];
    for (let j = 1; j <= songsPerAlbum; j++) {
      tracks.push({
        name: `示例歌曲 ${j}.mp3`,
        url: '#',
        lrcUrl: '#',
        size: 1024 * 1024 * (3 + Math.random() * 5), // 3-8 MB
        type: 'audio/mpeg',
        displayName: `示例歌曲 ${j}`,
        fileName: `sample-song-${j}`,
        id: `sample-${i}-${j}`
      });
    }
  ...
  for (let k = 1; k <= 8; k++) {
    rootTracks.push({
      ...
      url: '#',
      lrcUrl: '#',
      size: 1024 * 1024 * (2 + Math.random() * 3),
      type: 'audio/mpeg',
      displayName: `根目录歌曲 ${k}`,
      fileName: `root-song-${k}`,
      id: `root-${k}`
    });
  }
  ...
  return {
    ...
    repo: GITHUB_REPO,
    path: MUSIC_PATH || '根目录',
    message: '示例数据 - 请配置 GITHUB_TOKEN 访问真实仓库'
  };
}
...
// 修改 index.html 创建函数，添加配置说明
function createModernIndexHtml(filePath, musicData) {
  // 保持原有的 HTML 和 CSS 不变，但添加配置说明
...
<body>
    ...
    <div id="config-hint" style="
        ...
        font-size: 12px;
        max-width: 300px;
        display: ${musicData.totalAlbums === 0 ? 'block' : 'none'};
    ">
        <strong>配置提示:</strong> 请设置 GITHUB_TOKEN 环境变量以访问你的音乐仓库
    </div>
    <!-- 加载动画 -->
    ...
    <script>
        ...
        async function loadMusicData() {
            try {
                ...
                try {
                    ...
                    console.log('从静态文件加载的音乐数据:', data);
                    
                    // 检查数据格式
                    if (data && (data.albums || data.success)) {
                        const albums = data.albums || [];
                        console.log('从静态文件加载音乐数据成功:', albums.length, '个专辑');
                        ...
                        updateStats({
                            albums: albums,
                            totalSongs: data.totalSongs || albums.reduce((sum, album) => sum + album.tracks.length, 0),
                            totalAlbums: data.totalAlbums || albums.length
                        });
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                if (albums.length > 0 && albums.reduce((sum, album) => sum + album.tracks.length, 0) > 0) {
                    ...
                    // 尝试从 API 获取
                    try {
                        console.log('尝试从 API 获取音乐数据...');
                        const response = await fetch('/api/list-music');
                        if (response.ok) {
                            const data = await response.json();
                            if (data.success && data.albums) {
                                console.log('从API加载音乐数据成功:', data.albums.length, '个专辑');
        ...
        function renderAlbumLists() {
            if (musicData.albums.length === 0) {
                console.log('没有专辑数据可渲染');
                return;
            }
            
            console.log('渲染专辑列表，共', musicData.albums.length, '个专辑');
            
            // 清空列表
            desktopAlbumsList.innerHTML = '';
            mobileAlbumsList.innerHTML = '';
        ...
        function renderView(view) {
            ...
            switch(view) {
                case 'albums':
                    // 显示专辑网格或当前专辑的歌曲
                    if (musicData.currentAlbumTracks.length > 0) {
                        renderAlbumSongs(musicData.albums[musicData.currentAlbumIndex]);
                    } else {
                        renderAlbumsGrid();
        ...
        function renderAlbumSongs(album) {
            console.log('渲染专辑歌曲:', album.name, album.tracks.length, '首歌曲');
        ...
        function renderAllSongs() {
            ...
            allTracks.forEach((track, index) => {
                const item = createMusicListItem(track, index, {
                    name: track.albumName,
                    cover: musicData.albums[track.albumIndex]?.cover || null
                });
                musicList.appendChild(item);
            });
        }
        ...
        function updateCovers(album) {
            // 播放器封面
            if (album.cover) {
                playerCover.innerHTML = \`<img src="\${album.cover}" alt="\${album.name}">\`;
                playerModalCover.innerHTML = \`<img src="\${album.cover}" alt="\${album.name}">\`;
            } else {
