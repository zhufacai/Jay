...
<head>
    ...
    <style>
        ...
        * {
        ...
        .player-modal-cover:hover .cover-hint {
            opacity: 1;
        }
...
<body>
        ...
        <div class="player-modal-content">
            ...
            <div class="lyrics-view" id="lyrics-view">
                <div class="lyrics-view-header">
                    <div class="lyrics-view-title">歌词</div>
                    <button class="lyrics-view-close" id="lyrics-view-close">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
    ...
    <script>
        ...
        const lyricsViewClose = document.getElementById('lyrics-view-close');
        ...
        function initEventListeners() {
            ...
            if (modalPrevBtn) modalPrevBtn.addEventListener('click', prevTrack);
            if (modalNextBtn) modalNextBtn.addEventListener('click', nextTrack);
            if (modalShuffleBtn) modalShuffleBtn.addEventListener('click', toggleShuffle);
            if (modalRepeatBtn) modalRepeatBtn.addEventListener('click', toggleRepeat);
            ...
            // 模态框封面点击切换到歌词视图
            if (playerModalCover) playerModalCover.addEventListener('click', showLyricsView);
            
            // 模态框关闭
            if (playerModalClose) playerModalClose.addEventListener('click', closePlayerModal);
            
            // 歌词视图关闭
            if (lyricsViewClose) lyricsViewClose.addEventListener('click', showPlayerView);
        ...
        function updatePlayButtons() {
            if (musicData.isPlaying) {
                ...
                playBtn.title = '暂停';
                if (modalPlayBtn) modalPlayBtn.title = '暂停';
            } else {
                playIcon.className = 'fas fa-play';
                if (modalPlayIcon) modalPlayIcon.className = 'fas fa-play';
                playBtn.title = '播放';
                if (modalPlayBtn) modalPlayBtn.title = '播放';
            }
            
            // 更新控制按钮颜色
            if (shuffleBtn) shuffleBtn.style.color = musicData.isShuffle ? 'var(--primary)' : 'var(--text-secondary)';
            if (modalShuffleBtn) modalShuffleBtn.style.color = musicData.isShuffle ? 'var(--primary)' : 'var(--text-secondary)';
            if (repeatBtn) repeatBtn.style.color = musicData.isRepeat ? 'var(--primary)' : 'var(--text-secondary)';
            if (modalRepeatBtn) modalRepeatBtn.style.color = musicData.isRepeat ? 'var(--primary)' : 'var(--text-secondary)';
        }
        ...
        function updateCovers(album) {
            ...
            if (playerModalCover) playerModalCover.innerHTML = coverHtml + '<div class="cover-hint">点击查看歌词</div>';
        ...
        // 渲染无歌词状态
        function renderNoLyrics(message = '暂无歌词') {
