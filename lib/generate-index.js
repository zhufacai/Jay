...
async function fetchMusicFiles() {
  try {
    ...
    // 处理根目录下的音乐文件
    const rootCoverFile = files.find(file => {
  const ext = path.extname(file.name).toLowerCase();
  const fileNameWithoutExt = path.basename(file.name, ext);
  return ['.jpg', '.jpeg', '.png', '.gif'].includes(ext) && 
         fileNameWithoutExt.toLowerCase() === 'cover'; // 根目录使用 cover.jpg 等
});
...
if (rootMusicFiles.length > 0) {
  const rootAlbum = {
    name: '根目录',
    path: MUSIC_PATH || '',
    cover: rootCoverFile ? `https://raw.githubusercontent.com/${GITHUB_REPO}/${BRANCH}/${MUSIC_PATH || ''}/${encodeURIComponent(rootCoverFile.name)}` : null,
...
// 如果没有同名封面，再查找任意封面图片
if (!coverFile) {
  coverFile = folderFiles.find(file => 
    ['.jpg', '.jpeg', '.png', '.gif'].includes(path.extname(file.name).toLowerCase())
          ...
          if (musicFiles.length > 0) {
            const coverPath = coverFile ? `${folder.path}/${coverFile.name}` : null;
const album = {
  name: folder.name,
  path: folder.path,
  cover: coverPath ? `https://raw.githubusercontent.com/${GITHUB_REPO}/${BRANCH}/${encodeURIComponent(coverPath)}` : null,
...
<body>
  ...
  ? `<img src="${album.cover}" alt="${album.name}" onerror="this.onerror=null; this.src=''; this.style.display='none'; this.parentNode.innerHTML='<i class=\\"fas fa-compact-disc\\"></i>';">`
  : `<i class="fas fa-compact-disc"></i>`;
