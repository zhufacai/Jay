...
async function fetchMusicFiles() {
  try {
    ...
    // 如果提供了 GITHUB_TOKEN，直接调用 GitHub API
    if (GITHUB_TOKEN && GITHUB_REPO !== 'username/repo') {
      console.log('使用 GitHub API 获取音乐文件...');
      musicData = await getMusicFilesFromGitHub();
    } else {
      console.log('未提供有效的 GitHub 配置，生成空数据');
      musicData = {
        success: true,
        albums: [],
        totalSongs: 0,
        ...
        repo: GITHUB_REPO,
        path: MUSIC_PATH || '根目录',
        message: '请配置 GITHUB_REPO 和 GITHUB_TOKEN 环境变量'
      };
    }
...
async function getMusicFilesFromGitHub() {
  try {
    ...
    // 处理每个文件夹（专辑）- 最多处理20个文件夹以免超时
    const maxFolders = Math.min(folders.length, 20);
